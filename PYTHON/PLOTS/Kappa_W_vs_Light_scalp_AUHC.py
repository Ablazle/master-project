...
# Uses all CSV files in O:/Tech_NeuroData/Code/New_1CH_AUHCologne/OUS_files/sleepstages_scalp
...
# 1) File definitions
# ──────────────────────────────────────────────────────────────────────────────
base_path = r"O:\Tech_NeuroData\Code\New_1CH_AUHCologne\OUS_files\sleepstages_scalp"
file_paths = glob.glob(os.path.join(base_path, "*.csv"))
if not file_paths:
    raise FileNotFoundError("No CSV files found in the specified directory.")

# Use filenames as method labels (stripped of .csv and common prefixes/suffixes)
...
# ──────────────────────────────────────────────────────────────────────────────
for lbl, p in zip(labels, file_paths):
    df = pd.read_csv(p)
...
# ──────────────────────────────────────────────────────────────────────────────
# 3) Hardcoded Subject-to-Group Mapping
# ──────────────────────────────────────────────────────────────────────────────
hc_subjects = {
...
for a, b in itertools.combinations(labels, 2):
    ...
    for subject_id, subject_df in merged.groupby("Subject"):
        ...
        if group is None:
            continue  # skip if subject not mapped
...
# Optionally, print summary of all computed pairwise kappas
print("\nPairwise Cohen's κ scores by group:")
for row in df_k_grouped.itertuples(index=False):
    print(f"{row.Pair} | Subject: {row.Subject} | Group: {row.Group} | Kappa: {row.Kappa:.2f}")
